...
def _parse_rule(rule: ET.Element, source_ref: str, context_type: Optional[str] = None) -> Optional[RawTask]:
    ...
    if primary_action is None:
        return None
    primary_action = _v5_select_primary_action(primary_action, then_action_records)
...
V5_PAYLOAD_ACTIONS: set[str] = {
    'Describe', 'Provide Contact Details', 'Provide Information',
}

V5_SUPPORT_RENDER_ACTIONS: set[str] = {
    'Demonstrate Compliance', 'Comply With Obligations', 'Measures Already Taken',
    'Measures Proposed', 'Measures Taken/Proposed', 'Define Processing Scope',
    'Take Risk Into Account',
}

V5_LEGAL_STATE_ACTIONS: set[str] = {
    'Bear Legal Responsibility',
}

V5_WEAK_PRIMARY_ACTIONS: set[str] = {
    'Define', 'Implement', 'Verify', 'Demonstrate', 'ComplyWith',
    'TakeIntoAccount', 'Protect', 'Monitor', 'AdviseOn', 'AttachTo',
    'CooperateWith', 'AssistFor', 'Publish',
}

ACTION_SIGNATURES['Transmit'] = {'actor': 1, 'content': (2,), 'recipient': 3}
ACTION_SIGNATURES['WriteIn'] = {'actor': 1, 'content': (2,)}


def _v5_action_priority(action: ActionRecord) -> int:
    verb = action.verb
    if not verb or verb in NOTE_ONLY_CONCEPTS:
        return -100

    score = 0
    if verb == 'Document':
        score += 130
    elif verb not in V4_SUPPORT_ACTIONS:
        score += 100
    else:
        score += 20

    if verb in {'Communicate', 'Transmit', 'Notify', 'Request', 'ReceiveFrom', 'Delete', 'Rectify', 'Document', 'Limit', 'Lodge', 'PayFor'}:
        score += 15
    if action.recipient:
        score += 10
    if action.data_objects:
    ...
    if action.negated:
        score -= 5
    if verb in V5_WEAK_PRIMARY_ACTIONS:
        score -= 10
    return score


def _v5_select_primary_action(primary_action: ActionRecord, then_action_records: list[ActionRecord]) -> ActionRecord:
    candidates = [copy.deepcopy(primary_action)]
    primary_sig = _v4_action_signature(primary_action)
    for action in then_action_records:
        if _v4_action_signature(action) == primary_sig:
            continue
        candidates.append(copy.deepcopy(action))

    best = max(candidates, key=_v5_action_priority)
    if (
        primary_action.verb in V5_WEAK_PRIMARY_ACTIONS
        and _v5_action_priority(best) > _v5_action_priority(primary_action)
    ):
        return best
    return primary_action


def _v5_pick_action_anchor(primary_action: ActionRecord, action_var: Optional[str]) -> str:
    if primary_action.refs:
        return primary_action.refs[0]
    return _v4_pick_action_anchor(primary_action, action_var)
...
def _v4_parse_formula(
    ...
    atom_by_first_ref: dict[str, list[ET.Element]] = defaultdict(list)
    atom_by_first_ref_then: dict[str, list[ET.Element]] = defaultdict(list)
    atom_by_first_ref_if: dict[str, list[ET.Element]] = defaultdict(list)
    for atom in all_atoms:
        refs = _atom_keyrefs(atom)
        if refs:
            atom_by_first_ref[refs[0]].append(atom)
    for atom in then_atoms:
        refs = _atom_keyrefs(atom)
        if refs:
            atom_by_first_ref_then[refs[0]].append(atom)
    for atom in if_atoms:
        refs = _atom_keyrefs(atom)
        if refs:
            atom_by_first_ref_if[refs[0]].append(atom)
    ...
    primary_action = None
    if primary_refs:
        if then_action_records:
            primary_action = _find_action_by_refs(primary_refs, then_action_records, atom_by_first_ref_then, var_registry)
            if primary_action is None:
                primary_action = _resolve_negated_action(primary_refs, then_atoms, then_action_records, atom_by_first_ref_then, var_registry)
        if primary_action is None:
            primary_action = _find_action_by_refs(primary_refs, action_records, atom_by_first_ref, var_registry)
        if primary_action is None:
    ...
    primary_action = _v5_select_primary_action(primary_action, then_action_records)
    if primary_action.refs:
        primary_refs = list(primary_action.refs)
    ...
    branch_type, branches = _v4_detect_branches(
        ...
        atom_by_first_ref_then if then_action_records else atom_by_first_ref,
        then_action_records if then_action_records else action_records,
        primary_action,
        var_registry,
    )
    ...
    return FormulaV4(
        ...
        action_anchor=_v5_pick_action_anchor(primary_action, action_var),
        conditions=conditions,
        notes=notes,
...
def _v5_article_root(source: str) -> str:
    match = re.match(r'(Art\. \d+)', source)
    return match.group(1) if match else source


def _v5_dedup_notes(notes: list[str]) -> list[str]:
    result: list[str] = []
    seen: set[str] = set()
    for note in notes:
        clean = note.strip()
...
def _v5_branch_is_payload(branch: dict, base_action: str) -> bool:
    for task in branch.get('tasks', []):
        action = task.get('action', '')
        if action.startswith(f'{base_action}:'):
            continue
        if action in V5_SUPPORT_RENDER_ACTIONS or action in V5_PAYLOAD_ACTIONS:
            continue
        if action.startswith('Describe Measures') or action.startswith('Define Processing Scope:'):
            continue
        return False
    return True


def _v5_attach_payload(anchor: dict, payload_task: dict):
    if payload_task.get('data_objects'):
        for item in payload_task['data_objects']:
            _append_unique(anchor['data_objects'], item)
    else:
        payload_label = payload_task.get('action')
        if payload_label:
            _append_unique(anchor['notes'], f"Include: {payload_label}")
    for note in payload_task.get('notes', []):
        if note.startswith('Applies when:'):
            continue
        _append_unique(anchor['notes'], note)


def _v5_is_breach_documentation(task: dict) -> bool:
    labels = [str(item) for item in task.get('data_objects', [])]
    labels.extend(str(note) for note in task.get('notes', []))
    joined = ' '.join(labels).lower()
    if task.get('action') == 'Document':
        return 'breach' in joined or 'personal data breach' in joined
    if 'outcome should be verifiable' not in joined:
        return False
    if task.get('action') in V5_SUPPORT_RENDER_ACTIONS or task.get('action') == 'Document':
        return 'breach' in joined or 'personal data breach' in joined
    return False


def _v5_is_documentation_duty(task: dict) -> bool:
    notes_joined = ' '.join(str(note).lower() for note in task.get('notes', []))
    if task.get('action') == 'Document':
        return True
    if 'outcome should be verifiable' not in notes_joined:
        return False
    if task.get('action') in V5_SUPPORT_RENDER_ACTIONS:
        return True
    return any(post.get('action') in V5_SUPPORT_RENDER_ACTIONS for post in task.get('post_actions', []))


def _v5_refine_task(task: dict):
    if task.get('action', '').startswith('Can '):
        task['action'] = task['action'][4:]
        _append_unique(task['notes'], 'Permitted under these conditions')

    if task.get('action') == 'Limit' and task.get('actor') == 'DataSubject' and task.get('recipient') == 'Controller':
        task['action'] = 'Request Restriction of Processing'

    if task.get('action') == 'Bear Legal Responsibility':
        task['action'] = 'Assess Liability for Damage'
        _append_unique(task['notes'], 'Represents a liability consequence rather than an operational step')

    if _v5_is_breach_documentation(task):
        task['action'] = 'Document Personal Data Breach'
        _append_unique(task['notes'], 'Document the facts, effects and remedial action related to the breach')
    elif _v5_is_documentation_duty(task):
        task['action'] = 'Document Information'

    filtered_post_actions = []
    for post_action in task.get('post_actions', []):
        action = post_action.get('action', '')
        if action.startswith('Can '):
            post_action = dict(post_action)
            post_action['action'] = action[4:]
        if action in V5_SUPPORT_RENDER_ACTIONS:
            _append_unique(task['notes'], f"Also address: {post_action.get('action')}")
            for item in post_action.get('data_objects', []):
                _append_unique(task['data_objects'], item)
            continue
        filtered_post_actions.append(post_action)
    task['post_actions'] = filtered_post_actions

    if task.get('action', '').startswith('Document '):
        task['notes'] = [
            note for note in task.get('notes', [])
            if note not in {'Notify the competent supervisory authority'}
        ]

    if task.get('branches') and all(_v5_branch_is_payload(branch, task.get('action', '')) for branch in task['branches']):
        for branch in task['branches']:
            label = branch.get('label')
            if label:
                _append_unique(task['notes'], f"Include: {label}")
            for branch_task in branch.get('tasks', []):
                for item in branch_task.get('data_objects', []):
                    _append_unique(task['data_objects'], item)
                action = branch_task.get('action', '')
                if action and action != task.get('action'):
                    _append_unique(task['notes'], f"Also address: {action}")
        task['branches'] = []
        task['gateway_type'] = 'annotation'

    task['notes'] = _v5_dedup_notes(task.get('notes', []))


def _v5_refine_tasks(processed: list[dict]) -> list[dict]:
    for task in processed:
        _v5_refine_task(task)

    anchors_by_article_actor: dict[tuple[str, str], list[dict]] = defaultdict(list)
    for task in processed:
        if task.get('bpmn_type') == 'bpmn:sendTask' or task.get('recipient'):
            anchors_by_article_actor[(_v5_article_root(task.get('source', '')), task.get('actor', ''))].append(task)

    kept: list[dict] = []
    for task in processed:
        article_root = _v5_article_root(task.get('source', ''))
        payload_like = task.get('action') in V5_PAYLOAD_ACTIONS or task.get('action') == 'Describe Measures'
        actor_key = (article_root, task.get('actor', ''))
        fallback_key = (article_root, 'Controller')
        if (
            payload_like
            and not task.get('recipient')
            and not task.get('branches')
            and (
                anchors_by_article_actor.get(actor_key)
                or (task.get('actor') == 'System' and anchors_by_article_actor.get(fallback_key))
            )
        ):
            anchor_candidates = anchors_by_article_actor.get(actor_key) or anchors_by_article_actor.get(fallback_key) or []
            anchor = anchor_candidates[0]
            _v5_attach_payload(anchor, task)
            continue
        kept.append(task)

    for task in kept:
        task['notes'] = _v5_dedup_notes(task.get('notes', []))
        if task.get('action', '').endswith('within deadline?'):
            task['timer'] = None
    return kept


def extract_gdpr_structural(xml_path: str, target_article: str) -> list[dict]:
    print(f"[v5] Strukturelle Extraktion fuer '{target_article}'...")
    ...
    if not statement_ids:
        print(f"[v5] Keine Statements fuer '{target_article}' gefunden.")
        return []
    ...
    processed = _v5_refine_tasks(processed)
    ...
    if processed and article_notes:
        processed[0]['article_notes'] = article_notes
    print(f"[v5] {len(processed)} Aufgaben extrahiert.")
    return processed
